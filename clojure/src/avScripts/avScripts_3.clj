;java -cp .:classes -jar D:/mani/dev/opt/clojure-1.6.0/clojure-1.6.0.jar -i D:/mani/dev/project/z_clojure/src/avScripts/avScripts_3.clj -m avScripts.avScripts_3 3
;(load-file "D:/mani/dev/project/z_clojure/src/avScripts/avScripts_3.clj")

(ns avScripts.avScripts_3
...
(defn ffmpegEncodeMpeg2
  ([inFile outFile qscale]
   (def cmds {
              :1 ["Convert to mpeg2. -qscale 2-31 is quality, 2 being highest" 
                  (str ffmpeg " -i %s -codec:v mpeg2video -qscale:v %s -codec:a mp2 -b:a 192k %s")]
              })
   (let [x (second (cmds :1))]
     (executeSH (spliter (format x inFile qscale outFile)))))
  ([fileMap crf]
...
(defn ffmpegEncodeX265 
  ([inFile outFile crf]
   ...
   (let [x (second (cmds :2))]
     (executeSH (spliter (format x inFile crf outFile)))))
  ([fileMap crf]
...
(defn ffmpegEncodeX264 
  ...
  ([]
   ...
   (println "Provide CRF : ")
   (def crf (read-line))
   (def outFile (str (.substring inFile 0 (.lastIndexOf inFile ".")) "_f_x264_" crf  ".mkv"))
   (ffmpegEncodeX264 inFile outFile crf))
  )
...
(defn ffmpegEncodeForHDReadyTV 
  ...
  ([fileMap]
   (println "Provide CRF : ")
   (def crf (read-line))
   (loopEncode ffmpegEncodeForHDReadyTV fileMap crf))
  ([]
   (println "Provide input file : ")
   (def inFile (clojure.string/replace (read-line) #"\\" "/"))
   (println "Provide CRF : ")
...
(defn ffmpegImport
  ([inFile srtFile outFile]
   ...
   (let [x (second (cmds :1))]
     (executeSH (spliter (format x inFile srtFile outFile)))))
  ([]
   (println "Provide input file : ")
   (def inFile (clojure.string/replace (read-line) #"\\" "/"))
...
(defn ffmpegVolume 
  ([inFile outFile db]
   ...
   (let [x (second (cmds :1))]
     (executeSH (spliter (format x inFile db outFile)))))
  ([fileMap db]
...
(defn handbrakeEncodeX265 
  ([inFile outFile crf]
   ...
   (let [x (second (cmds :2))]
     (executeSH (spliter (format x inFile outFile crf)))))
  ([fileMap crf]
...
(defn -main [args]
  ...
  (def srcFile "D:/mani/video/compressed/<>.mp4")
  (def outFile "D:/mani/video/compressed/fsdafas_.mkv")
  (def fromExt "")
  ...
  (def cmdMap [[(str "ffmpeg -> x265 convert all in " dirPath) #(ffmpegEncodeX265 (fileMap dirPath fromExt "_f_x265_23.mkv") "23")]
               ;[(str "ffmpeg -> x265 convert " srcFile " -> " outFile) #(ffmpegEncodeX265 srcFile outFile "21")]
               [(str "ffmpeg -> x265 convert ") #(ffmpegEncodeX265)]
               [(str "ffmpeg -> x264 convert all in " dirPath) #(ffmpegEncodeX264 (fileMap dirPath fromExt "_f_x264_21.mkv") "21")]
               ;[(str "ffmpeg -> x264 convert " srcFile " -> " outFile) #(ffmpegEncodeX264 srcFile outFile "21")]
               [(str "ffmpeg -> x264 convert ") #(ffmpegEncodeX264)]
               [(str "ffmpeg -> cut " srcFile) #(ffmpegCut srcFile)]
               ;[(str "ffmpeg -> import subtitle " srcFile  " + " srtFile " -> " outFile) #(ffmpegImport srcFile srtFile outFile)]
               [(str "ffmpeg -> import subtitle ") #(ffmpegImport)]
               [(str "ffmpeg -> concat videos in file " concatFilesIn " -> " outFile) #(ffmpegConcat concatFilesIn outFile)]
               [(str "ffmpeg -> inc-dec volume of audio all in " dirPath) #(ffmpegVolume (fileMap dirPath fromExt "_v15") 15)]
               ;[(str "ffmpeg -> inc-dec volume of audio " srcFile " -> " outFile) #(ffmpegVolume srcFile outFile 12)]
               [(str "ffmpeg -> inc-dec volume of audio ") #(ffmpegVolume)]
               [(str "ffmpeg -> convert to HD ready TV ") #(ffmpegEncodeForHDReadyTV)]
               [(str "ffmpeg -> convert to HD ready TV in " dirPath) #(ffmpegEncodeForHDReadyTV (fileMap dirPath fromExt "_f_x264_hdReady_31.mkv"))]
               [(str "ffmpeg -> mpeg2 convert all in " dirPath) #(ffmpegEncodeMpeg2 (fileMap dirPath fromExt "_f_mpeg2_5.mpg") "5")]
               [(str "handbrake -> x265 convert all in " dirPath) #(handbrakeEncodeX265 (fileMap dirPath fromExt "_h_x265_21.mkv") "21")]
               [(str "handbrake -> x265 convert " srcFile " -> " outFile) #(handbrakeEncodeX265 srcFile outFile "21")] ])
