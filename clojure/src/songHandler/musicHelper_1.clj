(ns songHandler.musicHelper_1
    (use [clojure.java.shell :only [sh]])
    (load-file "../utils/fileUtils.clj")
    (use [utils.fileUtils :exclude [-main]])
    (load-file "./musicScript.clj")
    (use songHandler.musicScript)
    (load-file "./clj-id3/parser.clj")
    (use clj-id3.parser)
    (:gen-class)
)
...
(defn newWriteID3tagMp3ToM4a
    [mp3File, m4aFile]
        (def m (parse-mp3-file mp3File))
        (def newId3tag {:artist (getMapValue m :artist)
                        :album (getMapValue m :album)
                        :composer (getMapValue m :composer)
                        :lyricist (getMapValue m :lyricist)
                        :title (getMapValue m :title)
                        :year (getMapValue m :year)
                        :genre (getMapValue m :genre)
                        :comment (getMapValue m :comment)
                        :cover (getMapValue m :cover)})
        (println "new id3 tag " newId3tag)
        (writeM4aTag m4aFile (readMp3tag mp3File)))
